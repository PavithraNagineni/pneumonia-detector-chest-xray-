...
app = FastAPI(
    ...
    description="ResNet-50 deep learning model for chest X-ray pneumonia detection",
    version="1.0.0",
)

# ── CORS — required for frontend HTML to call API ────────────────────────────
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def load_model():
    ...
    try:
        ...
        logger.warning(f"⚠️  No trained model found ({e}). Using untrained weights — train first for real predictions.")
